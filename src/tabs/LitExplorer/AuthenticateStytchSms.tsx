...
interface AuthenticateStytchSmsProps {
  onBack: () => void;
...
type StytchSmsStep = "send-otp" | "verify-otp";

const DEFAULT_AUTH_SERVICE_BASE_URL = "https://naga-auth-service.onrender.com";

const AuthenticateStytchSms: React.FC<AuthenticateStytchSmsProps> = ({
  onBack,
  onAuthSuccess,
}) => {
  const [step, setStep] = useState<StytchSmsStep>("send-otp");
  ...
  const sendOtp = async () => {
    try {
      ...
      setError(null);
      setStatus("Sending OTP to your phone...");
      ...
      setStatus(
        `✅ OTP sent successfully to ${formattedPhone}! Check your phone for the verification code.`
      );
      setStep("verify-otp");
    } catch (error: any) {
      // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    try {
      ...
      if (authData?.metadata?.userId && formattedPhone) {
        localStorage.setItem(
          `stytchSmsOtp:${formattedPhone}`,
          authData.metadata.userId
        );
      }

      // Create user data for Stytch SMS authentication
      ...
      >
            ...
            <div style={{ textAlign: "center", marginTop: "1rem" }}>
              <button
                onClick={() => {
                  ...
                  setOtpCode("");
                  setError(null);
                  setStatus("You can now send a new SMS code.");
                }}
                style={{
                  background: "none",
                  border: "none",
        ...
        >
          ...
          <ol style={{ margin: 0, paddingLeft: "1.2rem" }}>
            ...
            <li>Check your phone and enter the 6-digit code</li>
...
export default AuthenticateStytchSms;
