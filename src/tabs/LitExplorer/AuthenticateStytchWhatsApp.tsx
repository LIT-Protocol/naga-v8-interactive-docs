...
interface AuthenticateStytchWhatsAppProps {
  onBack: () => void;
...
interface AuthUser {
  ...
  method: "stytch-whatsapp";
  timestamp: number;
  authData?: any; // eslint-disable-line @typescript-eslint/no-explicit-any
  accountMethod: string;
}

type StytchWhatsAppStep = "send-otp" | "verify-otp";

const DEFAULT_AUTH_SERVICE_BASE_URL = import.meta.env.VITE_AUTH_SERVICE_BASE_URL || "https://naga-auth-service.onrender.com";

const AuthenticateStytchWhatsApp: React.FC<AuthenticateStytchWhatsAppProps> = ({
  onBack,
  onAuthSuccess,
}) => {
  const [step, setStep] = useState<StytchWhatsAppStep>("send-otp");
  ...
  const sendOtp = async () => {
    try {
      ...
      setStatus(
        `✅ OTP sent successfully to ${formattedPhone} via WhatsApp! Check your WhatsApp messages for the verification code.`
      );
      setStep("verify-otp");
    } catch (error: any) {
      // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const verifyOtp = async () => {
    try {
      ...
      const formattedPhone = formatPhoneNumber(phoneNumber);
      if (authData?.metadata?.userId && formattedPhone) {
        localStorage.setItem(
          `stytchWhatsAppOtp:${formattedPhone}`,
          authData.metadata.userId
        );
      }

      // Create user data for Stytch WhatsApp authentication
  ...
  return (
    <div style={{ padding: "2rem", maxWidth: "600px", margin: "0 auto" }}>
      ...
      >
        ← {step === "send-otp" ? "Back" : "Back to Phone"}
      </button>
      ...
      >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
            ...
            <div
              style={{
                flex: 1,
                height: "2px",
                backgroundColor: step === "verify-otp" ? "#10b981" : "#e5e7eb",
                transition: "all 0.3s",
              }}
            />
        ...
        {step === "send-otp" ? (
          ...
          <div>
            ...
            <button
              ...
              style={{
                ...
                padding: "16px",
                backgroundColor:
                  isSendingOtp || !phoneNumber ? "#9ca3af" : "#10b981",
                color: "white",
                border: "none",
                borderRadius: "8px",
                fontSize: "16px",
            ...
            >
              ...
              {isSendingOtp ? "Sending WhatsApp..." : "Send WhatsApp Code"}
            </button>
          </div>
        ) : (
          /* Verify OTP Step */
          <div>
            ...
            >
              ...
              <strong>{formatPhoneNumber(phoneNumber)}</strong> via WhatsApp
            </div>
            ...
            <div style={{ textAlign: "center", marginTop: "1rem" }}>
              <button
                onClick={() => {
                  ...
                  setOtpCode("");
                  setError(null);
                  setStatus("You can now send a new WhatsApp code.");
                }}
                style={{
                  background: "none",
                  border: "none",
        ...
        >
          ...
          <ol style={{ margin: 0, paddingLeft: "1.2rem" }}>
            ...
            <li>Check your WhatsApp and enter the 6-digit code</li>
...
export default AuthenticateStytchWhatsApp;
