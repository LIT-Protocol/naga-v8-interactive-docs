...
import { PermissionsDangerZone } from './PermissionsDangerZone';

interface PermissionsDashboardProps {
  disabled?: boolean;
}

export const PermissionsDashboard: React.FC<PermissionsDashboardProps> = ({ disabled = false }) => {
  const {
    permissionsContext,
  ...
  useEffect(() => {
    if (selectedPkp && !permissionsContext && !isLoadingPermissions) {
      console.log("🔄 Auto-loading permissions context for PKP:", selectedPkp.tokenId);
      loadPermissionsContext();
    }
  }, [selectedPkp, permissionsContext, isLoadingPermissions, loadPermissionsContext]);
  ...
  return (
    <>
      ...
      >
        ...
        >
          Manage what your PKP wallet can do. Add actions, addresses, and control permissions.
        </p>
      </div>
      ...
      >
        ...
        >
          ...
          <button
            ...
            style={{
              padding: "8px 16px",
              backgroundColor: isLoadingPermissions || disabled ? "#9ca3af" : "#3b82f6",
              color: "white",
              border: "none",
              borderRadius: "6px",
              fontSize: "12px",
              fontWeight: "500",
              cursor: isLoadingPermissions || disabled ? "not-allowed" : "pointer",
            }}
          >
