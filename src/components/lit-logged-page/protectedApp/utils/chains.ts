/**
 * Supported Chains & Custom Chain Registry
 *
 * Purpose:
 *  - Provide default chain configurations used across the protected app
 *  - Allow users to add and manage custom chains that are cached locally
 *
 * Usage:
 *  - Import `getAllChains()` to retrieve a merged view of defaults and custom chains
 *  - Use `addCustomChain()` / `removeCustomChain()` to manage custom entries
 *  - Use `isCustomChain(slug)` to differentiate custom entries in the UI
 *
 * Notes:
 *  - Custom chains are stored in localStorage under the key defined by
 *    `CUSTOM_CHAINS_STORAGE_KEY`
 *  - We keep `SUPPORTED_CHAINS` exported for backwards compatibility
 */
